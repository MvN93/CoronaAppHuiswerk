...
public class ReserveringenManager {
    ...
    void neemReserveringAan(Persoon persoon, LocalTime beginTijdReservering, LocalTime eindTijdReservering) {
        ...
        while((vrijeTafelGevonden == false) && tafelnummer < horecaGelegenheid.getAantalTafels()) {
            ...
            if(tafelIsVrij == true) { //&& als genoeg plek aan tafel (dit moet nog toegevoegd worden)
                Reservering nieuweReservering = new Reservering(beginTijdReservering, eindTijdReservering, persoon.getNaam(), vindtTafelHorendeBijTafelnummer(tafelnummer, horecaGelegenheid));
    ...
    boolean ingevoerdeTijdsuurLigtInVolTijdsvlak(int ingevoerdeTijd, int beginTijdTijdsvlak, int eindTijdTijdsvlak){
        ...
        //!!Dus daar oplossing voor verzinnen; misschien via ingebouwde functionaliteit van DateTime
